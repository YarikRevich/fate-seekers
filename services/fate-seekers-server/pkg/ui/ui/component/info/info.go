...
var (
	// GetInstance retrieves instance of the info component, performing initial creation if needed.
	GetInstance = sync.OnceValue[*InfoComponent](newInfoComponent)
)

// InfoComponent represents component, which contains info statement.
...
func (pc *InfoComponent) SetText(value string) {
	pc.text.Label = value
}

// SetCloseCallback modified close callback in the container.
func (pc *InfoComponent) SetCloseCallback(callback func()) {
	pc.closeCallback = callback
}

// GetContainer retrieves container widget.
func (pc *InfoComponent) GetContainer() *widget.Container {
	return pc.container
}

// newInfoComponent creates info component.
func newInfoComponent() *InfoComponent {
	var result *InfoComponent
	...
	buttonsContainer.AddChild(widget.NewButton(
		...
		widget.ButtonOpts.Text(
			translation.GetInstance().GetTranslation("server.info.close"),
			generalFont,
